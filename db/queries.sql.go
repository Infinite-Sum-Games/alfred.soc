...
type VerifyRepositoryQueryParams struct {
	InstallationID pgtype.Int8 `json:"installation_id"`
	Url            string      `json:"url"`
}

func (q *Queries) VerifyRepositoryQuery(ctx context.Context, db DBTX, arg VerifyRepositoryQueryParams) (string, error) {
	row := db.QueryRow(ctx, verifyRepositoryQuery, arg.InstallationID, arg.Url)
	var name string
	err := row.Scan(&name)
	return name, err
}
